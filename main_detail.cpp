...
bool preview = false;
bool try_cuda = false;
double work_megapix = 0.6;  // 设置图像注册步骤的分辨率。 Mpx
double seam_megapix = 0.1;  // 设置接缝估计步骤的分辨率。 Mpx
double compose_megapix = -1;    // 设置compose步骤分辨率。
float conf_thresh = 1.f;    // 设置判断两张图像是否属于同一全景的置信度阈值。如拼接失败可降低此thres。
#ifdef HAVE_OPENCV_XFEATURES2D
string features_type = "surf";
float match_conf = 0.65f;
#else
string features_type = "orb";
float match_conf = 0.3f;    // 设置特征匹配步骤的置信度。
#endif
string matcher_type = "homography";     // 透视变换选homo，仿射选aff，这里应该用homo,否则core dump
string estimator_type = "homography";
string ba_cost_func = "ray";    // 指定捆绑调整（Bundle Adjustment）的代价函数，用于优化homo，可以减少图像的错位和畸变
string ba_refine_mask = "xxxxx";    // 设置BA优化掩码,5位，<fx><skew><ppx><aspect><ppy>，x表示优化该参数，_表示不优化
bool do_wave_correct = true;    
WaveCorrectKind wave_correct = detail::WAVE_CORRECT_HORIZ;  // 波性校正，解决图像波纹失真
bool save_graph = false;    //保存匹配图
std::string save_graph_to;  //匹配图保存路径
string warp_type = "spherical"; // 投影变换类型，全景选spherical或cylindrical
int expos_comp_type = ExposureCompensator::GAIN_BLOCKS; //曝光补偿方法（不影响拼接结果
int expos_comp_nr_feeds = 1;        //曝光补偿的迭代次数？
int expos_comp_nr_filtering = 2;    //曝光补偿增益的过滤迭代次数？
...
int main(int argc, char* argv[])
{
    ...
    vector<std::string> warp_img_path;
   
    for (int i = 0; i < num_images; ++i){
        std::string path = "../res/warped_img" + std::to_string(i) + ".jpg";
        warp_img_path.push_back(path);
        imwrite(warp_img_path[i],  images_warped[i]);
